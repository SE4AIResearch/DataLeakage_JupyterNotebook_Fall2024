  ...
  const uriPaths = {
    scripts: [
      webview.asWebviewUri(
        vscode.Uri.joinPath(extensionUri, 'media', 'button', 'settings.js'),
      ),
    ],
    styles: [
      // list the styles in the order to load them, index 0 is loaded first
      webview.asWebviewUri(
        vscode.Uri.joinPath(extensionUri, 'media', 'button', 'reset.css'),
      ),
      webview.asWebviewUri(
        vscode.Uri.joinPath(extensionUri, 'media', 'button', 'vscode.css'),
      ),
      webview.asWebviewUri(
        vscode.Uri.joinPath(extensionUri, 'media', 'button', 'main.css'),
      ),
      webview.asWebviewUri(
        vscode.Uri.joinPath(extensionUri, 'media', 'button', 'prio.css'),
      ),
      ...
      <body>
        ...
        ${uriPaths.scripts.reduce(
          (html, path) =>
            html +
            '\n' +
            `<script defer nonce="${data.nonce}" src="${path}"></script>`,
          '',
        )}
      </body>
    </html>`;
